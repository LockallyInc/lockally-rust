...
/// struct for typed errors of method [`v1_tenant_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V1TenantGetError {
    Status401(models::Problem),
    Status403(models::Problem),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`v1_usage_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V1UsageGetError {
    Status401(models::Problem),
    Status403(models::Problem),
    UnknownValue(serde_json::Value),
}
...
pub async fn v1_tenant_get(configuration: &configuration::Configuration, ) -> Result<models::Tenant, Error<V1TenantGetError>> {

    let uri_str = format!("{}/v1/tenant", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<V1TenantGetError> = serde_json::from_str(&content).ok();
...
pub async fn v1_usage_get(configuration: &configuration::Configuration, ) -> Result<models::V1UsageGet200Response, Error<V1UsageGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::V1UsageGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::V1UsageGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<V1UsageGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
